...
if df.index.duplicated().any():
    # Option 1: Drop duplicates in the index
...
# Create a temporary 'Month-Year' column without modifying df
month_year_column = df['Start Month'].fillna("Unknown") + ' ' + df['Start Year'].fillna("Unknown").astype(str)
...
def sort_key(month_year):
    try:
        ...
        # Handle entries that don't match "Month Year" format by placing them last
        return (float('inf'), float('inf'))

# Extract unique month-year combinations and sort them
month_years = sorted([date for date in month_year_column.unique() if date != "Unknown"], key=sort_key)

# Select slider for month-year range
if month_years:  # Ensure the list is not empty
    ...
    # Filter based on the selected month-year range
    ...
    # Filtered DataFrame based on month-year order indices without modifying df
    filtered_df = df[
        month_year_column.apply(lambda x: sort_key(x)).between(start_index, end_index)
    ]

# Use filtered_df as needed, without modifying df
